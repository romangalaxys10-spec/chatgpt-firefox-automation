...
- Client renderer build is committed to `client/dist/` so the Electron fallback
  works right after a clone.
- Portability: `deepseek_service.js` resolves the Firefox binary/profile from
  `VIBE_FIREFOX_BIN` / `VIBE_FIREFOX_PROFILE`; `launch_electron.sh` uses
  script-relative paths.
- `/brainstorm-{chatgpt,qwen,deepseek}` skills ship with the app.
