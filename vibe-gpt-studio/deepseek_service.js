...
// Resolve the Firefox binary + live profile portably. Override via env vars on
// machines where the defaults don't apply:
//   VIBE_FIREFOX_BIN     -> path to a Playwright Firefox executable
//   VIBE_FIREFOX_PROFILE -> path to a live Firefox profile dir (cookies+storage)
const FIREFOX_BIN =
  process.env.VIBE_FIREFOX_BIN ||
  path.join(os.homedir(), '.cache/ms-playwright/firefox-1538/firefox/firefox');
const LIVE_PROFILE =
  process.env.VIBE_FIREFOX_PROFILE ||
  path.join(os.homedir(), 'snap/firefox/common/.mozilla/firefox/14n5fjgr.default');
